...
@bot.message_handler(commands=['bot_activity'], func=admin_lambda)
def bot_activity_handler(m):
    ud = db.users.count_documents({'time': {'$gt': time.time() - 60*60*24}})
    uw = db.users.count_documents({'time': {'$gt': time.time() - 60*60*24*7}})
    u2w = db.users.count_documents({'time': {'$gt': time.time() - 60*60*24*14}})

    cd = db.globalchats.count_documents({'time': {'$gt': time.time() - 60*60*24}})
    cw = db.globalchats.count_documents({'time': {'$gt': time.time() - 60*60*24*7}})
    c2w = db.globalchats.count_documents({'time': {'$gt': time.time() - 60*60*24*14}})

    tts = f'📊Активность бота:\n'
    tts += f'💬Чаты: D: {cd} W: {cw}\n'
    tts += f'👤Пользователи: D: {ud} W: {uw}\n'
...
def announce(m):
    ...
    try:
        ...
        print(traceback.format_exc())
        i = -1
    bot.send_message(m.chat.id, f"Сообщение успешно получило {i}/{db.globalchats.count_documents({'active':True})} чатиков")

def hypercast(text):
    chats_ids = db.globalchats.find({'active': True})
    i = 0
    for doc in list(chats_ids):
        print(f'{i}/{len(list(chats_ids))}')
        try:
